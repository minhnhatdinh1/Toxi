...
const [discount, setDiscount] = useState({
 ...
 function applyCouponCode(code) {
  ...
  switch (c) {
    case "TOXI10":
      setDiscount({ type: "percent", value: 0.1 });
      setCouponMessage("Giảm 10%");
      break;

    case "VND50K":
      setDiscount({ type: "fixed", value: 50000 });
      setCouponMessage("Giảm 50.000đ");
      break;

    case "SHIPFREE":
      setDiscount({ type: "freeship", value: 0 });
 ...
 const discountAmount = useMemo(() => {
  ...
  const shippingAfter = discount.type === "freeship" ? 0 : shipping;
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          <div className="bg-white dark:bg-slate-850 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div className="overflow-x-auto custom-scrollbar">
              <table className="max-w-7x1 text-left border-collapse">
                <thead>
