...
export default function AdminPage() {
  ...
  const feed = [
    ...
    snapshotCourses[0] ? { time: fmtDate(snapshotCourses[0].createdAt || snapshotCourses[0].created_at), title: snapshotCourses[0].title || "Khóa học mới", desc: `Đã ghi nhận ${snapshotCourses.length} khóa học mới trong hệ thống.`, tone: "bg-emerald-500" } : null,
  ].filter(Boolean);
  const suggestion = (() => {
    const top = [...typeRevenue].sort((a, b) => b.amount - a.amount)[0];
    if (!top || top.amount <= 0) return "Chưa có dữ liệu doanh thu mua hàng thực tế để đưa ra gợi ý trong lúc này.";
    if (top.key === "BOOK") return "Doanh thu sách đang dẫn đầu. Bạn có thể đẩy mạnh combo sách + đề thi thử để tăng giá trị đơn hàng.";
    if (top.key === "COURSE") return "Khóa học đang là nhóm bán tốt nhất. Nên kết hợp quà tặng sách hoặc workbook để tăng tỷ lệ chốt đơn.";
      ...
      <main className="flex min-w-0 flex-1 overflow-hidden bg-slate-100">
        <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-10">
            ...
            <div className="mb-10 grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4">
              ...
              <Card icon="school" label="Doanh thu khóa học" value={money(courseRevenue)} delta={null} toneData={tone("COURSE")} footer={{ left: "Tổng khóa học đã bán", right: `${totalCourseUnits} khóa học`, progress: `${Math.max(courseShare, totalSalesRevenue > 0 ? 8 : 0)}%` }} />
              <Card icon="auto_stories" label="Doanh thu sách" value={money(bookRevenue)} delta={null} toneData={tone("BOOK")} footer={{ left: "Tổng sách đã bán", right: `${totalBookUnits} quyển`, progress: `${Math.max(bookShare, totalSalesRevenue > 0 ? 8 : 0)}%` }} />
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[860px] text-left text-sm">
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {recentOrders.length === 0 ? <tr><td colSpan={6} className="px-8 py-10 text-center text-sm text-slate-400">Chưa có đơn hàng để hiển thị.</td></tr> : recentOrders.map((o) => {
        ...
        <aside className="hidden w-[360px] border-l border-slate-200 bg-white 2xl:flex 2xl:flex-col">
          <div className="border-b border-slate-100 p-8">
            ...
            <p className="mt-1 text-sm font-medium text-slate-500">Cập nhật trực tiếp từ dữ liệu hiện có</p>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-8">
            ...
            <div className="mt-8 rounded-3xl border border-slate-100 bg-slate-50 p-6">
              ...
              <div className="mt-4 space-y-3">
                ...
                <div className="flex items-center justify-between text-sm"><span className="text-slate-500">Tổng sách đã bán</span><span className="font-black text-slate-900">{totalBookUnits}</span></div>
              </div>
            </div>
          </div>
        </aside>
