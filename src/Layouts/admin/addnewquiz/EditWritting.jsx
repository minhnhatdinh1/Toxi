...
const MOCK = {
  type:"sap-xep-viet",
  words:["我","学习","汉语","每天","在家"],
  correctOrder:"我每天在家学习汉语",
  pinyin:"Wǒ měitiān zàijiā xuéxí Hànyǔ.",
  meaning:"Tôi học tiếng Trung ở nhà mỗi ngày",
  explanation:"Trật tự: Chủ ngữ + Trạng ngữ thời gian + Trạng ngữ nơi chốn + Động từ + Tân ngữ.",
  hsk:"HSK 3",
};
...
export default function EditWritting() {
  ...
  const [words, setWords]         = useState(MOCK.words);
  const [newWord, setNewWord]     = useState("");
  const [correctOrder, setCorrectOrder] = useState(MOCK.correctOrder);
  const [pinyin, setPinyin]       = useState(MOCK.pinyin);
  const [meaning, setMeaning]     = useState(MOCK.meaning);
  ...
  const [form, setForm]           = useState({hsk:MOCK.hsk,autoGrade:false,shuffle:false,pinyin:true,score:1,seconds:120,status:"done"});
  const [recentList] = useState([
    {id:13,type:"Sắp xếp từ",    status:"done"},
    {id:14,type:"Viết đoạn văn", status:"draft"},
    {id:15,type:"Sắp xếp từ",   status:"done"},
  ]);
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-50">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <QuizPageHeader
          title={`Chỉnh sửa câu #${id||"??"} — ${TYPES.find(t=>t.value===activeType)?.label} (Viết)`}
          isEdit
                  ...
                  <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
                      ...
                      <div className="flex flex-wrap gap-2 p-4 bg-slate-50 rounded-xl border border-slate-200 min-h-16 mb-2">
                        {words.map((w,i)=>(
                          <span key={i} className="flex items-center gap-1.5 px-3 py-2 bg-white border border-slate-200 rounded-xl text-sm font-medium text-slate-700 shadow-sm group cursor-grab">
                            ...
                            {w}
                            <button onClick={()=>removeWord(i)} className="text-slate-300 hover:text-red-400 opacity-0 group-hover:opacity-100 transition ml-0.5">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <input className={inputCls} value={meaning} onChange={e=>setMeaning(e.target.value)}/>
                      </div>
                      <div>
                ...
                <>
                  <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
                    ...
                    <div>
                      ...
                      <textarea className={inputCls+" resize-none"} rows={3} value={promptText} onChange={e=>setPromptText(e.target.value)}
                        placeholder="Nhập từ khoá hoặc đoạn văn mẫu làm gợi ý..."/>
                    </div>
                    <div>
                    ...
                    <div>
                      <label className={labelCls}>Bài mẫu tham khảo</label>
                      <textarea className={inputCls+" resize-none"} rows={4} placeholder="Nhập bài viết mẫu..."/>
                    </div>
                  </div>
                </>
              )}
