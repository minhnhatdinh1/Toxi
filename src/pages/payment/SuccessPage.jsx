...
export default function SuccessPage() {
  ...
  const order = {
    ...
    itemTitle:
      ...
      "Hán ngữ giao tiếp sơ cấp: Chinh phục HSK 1-2",
  ...
  const cardTitle = isCourseOrder ? "THÔNG TIN KHÓA HỌC" : "THÔNG TIN SẢN PHẨM";
    ...
    <div style={{ minHeight: "100vh", fontFamily: "sans-serif", display: "flex", flexDirection: "column" }}>
      ...
      <nav style={{ background: "#1a2b5e", padding: "0 40px", height: 60, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <a href="/" style={{ display: "flex", alignItems: "center", gap: 10, textDecoration: "none" }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 9, color: "rgba(245,197,24,0.6)" }}>học để trí dụng</div>
          </div>
        </a>
        <div style={{ display: "flex", gap: 24 }}>
          {["Khóa học", "Tài liệu", "Cộng đồng", "Về chúng tôi"].map((label) => (
            <a key={label} href="#" style={{ color: "rgba(255,255,255,0.6)", fontSize: 12, fontWeight: 600, textDecoration: "none", letterSpacing: 1 }}>{label}</a>
          ))}
        </div>
      ...
      <div style={{ flex: 1, background: "linear-gradient(155deg, #c4956a 0%, #e8c49a 35%, #f0d4a8 50%, #7a8cb5 70%, #1a2b5e 100%)", padding: "48px 24px 0" }}>
        <div style={{ maxWidth: 500, margin: "0 auto", background: "#fff", borderRadius: 24, overflow: "hidden", boxShadow: "0 32px 80px rgba(26,43,94,0.25)" }}>
          <div style={{ padding: "40px 40px 24px", textAlign: "center" }}>
            <div style={{ width: 80, height: 80, borderRadius: "50%", background: "#4a7c59", margin: "0 auto 18px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 36, color: "#fff", animation: "pop .5s ease-out" }}>✓</div>
            <h2 style={{ margin: 0, fontSize: 30, fontWeight: 700, color: "#1a2b5e" }}>Cảm ơn bạn!</h2>
            <p style={{ color: "#8b7d5e", margin: "6px 0 0", fontSize: 13 }}>Thanh toán thành công</p>
          </div>
            ...
            <div style={{ padding: "16px", display: "flex", gap: 14, alignItems: "center" }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: "flex", gap: 12, fontSize: 11, color: "#8b7d5e" }}>
                  <span>Mã: <strong style={{ color: "#1a2b5e" }}>#{order.orderCode}</strong></span>
                  <span>Ngày: {order.paidAt}</span>
                  {!isCourseOrder ? <span>SL: {order.quantity}</span> : null}
                </div>
              </div>
              <div style={{ fontSize: 16, fontWeight: 800, color: "#4a7c59", whiteSpace: "nowrap" }}>{order.amount.toLocaleString("vi-VN")}đ</div>
            </div>
            <div style={{ padding: "10px 16px 14px", borderTop: "1px solid #f0ece6", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: 11, color: "#8b7d5e" }}>Tổng cộng:</span>
              <span style={{ fontSize: 18, fontWeight: 800, color: "#4a7c59" }}>{order.amount.toLocaleString("vi-VN")}đ</span>
            </div>
          </div>
          ...
          <div style={{ textAlign: "center", fontSize: 11, color: "#a09080", padding: "16px 24px 24px", lineHeight: 1.7 }}>
            Hệ thống đã gửi thông tin chi tiết vào email của bạn.<br />
            Nếu cần hỗ trợ: <strong>1900-TOXI-88</strong>
          </div>
        </div>
      </div>
      ...
      <footer style={{ background: "#1a2b5e", color: "rgba(255,255,255,0.5)", padding: "40px 48px 24px", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 28, marginTop: 40 }}>
        <div>
          ...
          <div style={{ fontSize: 12, lineHeight: 1.8 }}>Nền tảng học tiếng Trung hiện đại.</div>
        </div>
        {[["Khám phá", ["Thư viện Hán tự", "Luyện nghe Podcast", "Thi thử HSK"]], ["Hỗ trợ", ["Hướng dẫn thanh toán", "Chính sách bảo mật", "Câu hỏi thường gặp"]]].map(([title, links]) => (
          <div key={title}>
            ...
            {links.map((label) => <a key={label} href="#" style={{ display: "block", fontSize: 12, color: "rgba(255,255,255,0.4)", textDecoration: "none", marginBottom: 8 }}>{label}</a>)}
          </div>
        ))}
        <div>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", color: "rgba(255,255,255,0.7)", marginBottom: 12 }}>Kết nối</div>
          <div style={{ display: "flex", gap: 10 }}>
            {["f", "t", "Y"].map((label) => <div key={label} style={{ width: 34, height: 34, borderRadius: 8, background: "rgba(255,255,255,0.07)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, color: "rgba(255,255,255,0.5)", cursor: "pointer" }}>{label}</div>)}
          </div>
        </div>
